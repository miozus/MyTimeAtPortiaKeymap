...
数字键和功能键距离远时，还要找半天。所以干脆`后置转前置`，用键盘左下角的 <kbd>Ctrl</kbd> / <kbd>Alt</kbd> ，然后具体实现中附加后置动作。

而计数器的按钮，一行对称的五个按钮，只需要 <kbd>S</kbd> <kbd>F</kbd> 稍微移动一下就能点到同样常用的两侧，所以用 <kbd>Ctrl</kbd> 修饰， 中心按钮 <kbd>Alt</kbd> 修饰，左右都触发同样的效果。
...
1. 怎么提高识别的成功率和准确率？
   ...
   1. 静止性：无论鼠标和键盘怎么操作，颜色和坐标都不会变化。
      ...
      - 悬浮高亮：游戏中为了突出“强调”的视觉效果，鼠标悬浮到 UI 控件上时，控件会提高颜色的亮度。为了避免高亮导致丢失特征的影响，其实除了避开它或者扩大颜色判定阈值，我们还可以选择它失效时，其他同场景的其他特征作辅助验证，也可以让寄存器储存状态信息，表示“我还在这里”，在场景进出口做状态的记录或清除。
...
虽然增加了几个中间类，但责任是简短的，清晰的，必要的，热拔插的键距离远时，还要找半天。所以干脆`后置转前置`，用键盘左下角的 <kbd>Ctrl</kbd> / <kbd>Alt</kbd> ，然后具体实现中附加后置动作。
...
1. 怎么提高识别的成功率和准确率？
   ...
   1. 静止性：无论鼠标和键盘怎么操作，颜色和坐标都不会变化。
      ...
      - 悬浮高亮：游戏中为了突出“强调”的视觉效果，鼠标悬浮到 UI 控件上时，控件会提高颜色的亮度。为了避免高亮导致丢失特征的影响，其实除了避开它或者扩大颜色判定阈值，我们还可以选择它失效时，其他场景作辅助验证，也可以让寄存器储存状态信息，表示“我还在这里”，在场景进出口做状态的记录或清除。
...
1. 场景特征超过三个或太多，运行变慢，怎么办？
   ...
   为了代码可读性，我把相似控件都分门别类起函数名。有时查一个场景会把同类都查了一遍。
