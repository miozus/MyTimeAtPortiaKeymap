...
实际开发中，方向键和功能键很难融合在一起，因为 AHK 是单线程得，不能实现两个按键同时共享一个函数方法，虽然勉强通过拼接缓存区字符串表示一起按下多个键，但人物行动效果卡得 PPT，而挂起（假死）方向键，反而成了最优解。
...
1. 算法：没有什么高级函数和算法，就小学的一元一次方程，写两遍，计算二维坐标。
   ...
   - 背包格子 `8 * 4`
     ...
     充分利用 <kbd>9</kbd> <kbd>0</kbd> <kbd>-</kbd> <kbd>=</kbd> <kbd>P</kbd> ，select 函数分别传入参数 10 ~ 13 ，比 8 溢出的部分，刚好用作第 2 ~ 5 行，寄存器储存这个数字（我叫它梯子），乘以 Y 轴偏移量即可定位。
